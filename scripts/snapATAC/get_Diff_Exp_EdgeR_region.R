...
tissue[which(tissue %in% c("2A","3A"))] = "2A+3A"
tissue[which(tissue %in% c("12D","13B"))] = "12D+13B"
...
library(edgeR)
library(doParallel)
get_diff <- function(st1, st2, bigmat, cl, tiss) {
  mat = bigmat[,c(grep(tiss, colnames(bigmat),fixed = T))]
  mat = mat[,c(grep(st1, colnames(mat)), grep(st2, colnames(mat)))]
  ...
  show(head(out))
  write.csv(out,
            paste0("age_diff_edgeR.snap/region_specific/", cl, "_",tiss,"_",st1,"_vs_",st2, ".edger.txt"))
  return(out)
}
...
foreach(i = 19:max_cluster) %dopar% {
  ...
  if(is.null(mat) || ncol(mat) < 6) {
    ...
    for (tiss in c("3F+4E","5E+6E","7H+8H+9G","13D+14C","11E+11F+12E","8E+9H+8J+9J", "2A+3A", "12D+13B")) {
      if(length(grep(tiss,colnames(mat),fixed = T))>4) {
        cat(tiss, "\n")
        o_8wkvs18mo = get_diff("8wk","18mo",mat, cl, tiss)
        o_8wkvs9mo = get_diff("8wk","9mo",mat, cl,tiss)
        o_9movs18mo = get_diff("9mo","18mo",mat, cl,tiss)
      }
    }
  }
}
