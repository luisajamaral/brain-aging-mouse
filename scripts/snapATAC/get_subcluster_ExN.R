...
x.sp.sub <- x.sp[which(x.sp@cluster %in% c(13,15,17,21,22,27,31)), ]

#x.sp.sub <- x.sp[which(x.sp@cluster %in% c(3,9,12,19,25,28,29)), ]
#x.sp.sub <- x.sp[which(x.sp@cluster %in% c(4,6,20,30)), ]

outF = "../../analysis/snapATAC/all66/ExcN"
...
bin_size = 5000
tic("addBmatToSnap")
x.landmark.sp = addBmatToSnap(obj=x.landmark.sp, bin.size=bin_size)
toc()
