...
get_diff <- function(st1, st2, bigmat, cl, cell_num) {
  ...
  #show(head(out))
  write.csv(out,
            paste0("age_diff_edgeR.snap/",cell_num,"_", cl, "_",st1,"_vs_",st2, "each.edger.txt"))
  return(out)
}
...
stage_big = stage[which(cluster == "OLG")]
rep 
big_pmat = pmat
cl = "OLG"

#for (num_cells in c(250,500,1000,2500,5000,10000,25000)) {
for(num_cells in c(15000,20000)) {
  cat(num_cells, " each \n")

  s8wk = which(stage_big == "8wk")[sample.int(length(which(stage_big == "8wk")), num_cells)]
  s9mo = which(stage_big == "9mo")[sample.int(length(which(stage_big == "9mo")), num_cells)]
  s18mo = which(stage_big == "18mo")[sample.int(length(which(stage_big == "18mo")), num_cells)]
  samp = c(s8wk, s9mo, s18mo)
  pmat = big_pmat[samp,]
